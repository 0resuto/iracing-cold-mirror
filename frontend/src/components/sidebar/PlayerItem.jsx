...
export const PlayerItem = React.memo(function PlayerItem({ player, selectedLapId, setSelectedLap, onSelectLap }) {
  ...
  return (
    <div className="flex flex-col bg-zinc-950 border-l-4 border-sky-500 border-y border-r border-zinc-800 rounded-2xl overflow-hidden shadow-xl min-w-0">
      {/* Player Header Card */}
      <div
        onClick={() => setIsOpen(!isOpen)}
        style={{ padding: '14px 16px' }}
        className={`flex justify-between items-center cursor-pointer font-extrabold text-xs sm:text-sm transition-colors min-w-0 active:scale-[0.99] min-h-[48px] ${
      ...
      {/* Tracks List Level 2 */}
      {isOpen && (
        <div className="flex flex-col bg-black/60 min-w-0 gap-2" style={{ padding: '10px' }}>
          {sessionsCount === 0 ? (
