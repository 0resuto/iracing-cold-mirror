...
export const TelemetrySubChart = React.memo(({
  ...
  return (
    <div className="flex-1 flex flex-col relative group min-w-0" style={{ minHeight: minHeight && `${minHeight}px` }}>
      ...
      <div className="flex-1 mt-3">
        <ResponsiveContainer width="100%" height="100%">
          {type === "area" ? (
            <AreaChart data={data} syncId="telemetry" syncMethod="value" margin={{ top: 5, right: 10, left: 5, bottom: 0 }} onMouseEnter={() => { onChartActive(chartId); }}>
              ...
              <Line type="linear" dataKey={refDataKey} stroke="var(--color-zinc-400)" strokeWidth={1} dot={false} isAnimationActive={false} activeDot={false} />
            ...
            <LineChart data={data} syncId="telemetry" syncMethod="value" margin={{ top: 5, right: 10, left: 5, bottom: 0 }} onMouseEnter={() => { onChartActive(chartId); }}>
              <CartesianGrid fill="var(--color-brand-bg-deep)" strokeDasharray="3 3" stroke="rgba(255, 255, 255, 0.05)" vertical={false} />
              <XAxis dataKey="lap_dist_pct" hide={!showXAxis} type="number" domain={['dataMin', 'dataMax']} stroke="var(--color-zinc-400)" />
              <YAxis domain={domain} stroke="var(--color-zinc-400)" fontSize={9} tickCount={type==='line-speed' ? 5 : 3} width={35} />
              <Tooltip isAnimationActive={false} wrapperStyle={{ zIndex: 1000 }} content={<CustomTooltip chartId={chartId} activeChart={activeChart} />} />
