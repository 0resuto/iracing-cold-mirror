...
export const TelemetrySubChart = React.memo(({
  ...
  return (
    <div className={`flex-1 min-h-[${minHeight}px] flex flex-col relative group min-w-0`}>
      <div className="absolute left-10 top-0 text-[9px] text-[var(--color-text-muted)] font-bold tracking-widest z-10 group-hover:text-[var(--color-text-main)] transition-colors">
        {title}
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {type === "area" ? (
            <AreaChart data={data} syncId="telemetry" syncMethod="value" margin={{ top: 5, right: 10, left: 5, bottom: 0 }} onMouseEnter={() => { activeChartRef.current = chartId; }}>
              ...
              <XAxis dataKey="lap_dist_pct" hide={!showXAxis} type="number" domain={['dataMin', 'dataMax']} tickFormatter={showXAxis ? (val) => (val * 100).toFixed(0) + '%' : undefined} fontSize={9} minTickGap={30} stroke="var(--color-text-muted)" />
              <YAxis domain={domain} stroke="var(--color-text-muted)" fontSize={9} tickCount={3} tickFormatter={tickFormatter} width={35} />
              ...
              <Line type="linear" dataKey={refDataKey} stroke="var(--color-text-muted)" strokeWidth={1} dot={false} isAnimationActive={false} activeDot={false} />
              ...
              {sectorBoundaries.map((pct, i) => (
                <ReferenceLine key={`sector-${i}`} x={pct} stroke="var(--color-border-strong)" strokeDasharray="3 3" opacity={0.4} />
              ))}
            </AreaChart>
          ) : (
            <LineChart data={data} syncId="telemetry" syncMethod="value" margin={{ top: 5, right: 10, left: 5, bottom: 0 }} onMouseEnter={() => { activeChartRef.current = chartId; }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border-subtle)" vertical={false} />
              <XAxis dataKey="lap_dist_pct" hide={!showXAxis} type="number" domain={['dataMin', 'dataMax']} stroke="var(--color-text-muted)" />
              <YAxis domain={domain} stroke="var(--color-text-muted)" fontSize={9} tickCount={type==='line-speed' ? 5 : 3} width={35} />
              <Tooltip isAnimationActive={false} content={<CustomTooltip chartId={chartId} activeChartRef={activeChartRef} />} />
